...
u32 display_x = 800, display_y = 480, display_x_pitch = 0, display_offset = 0;
...
void gba_upscale(uint32_t *to, uint32_t *from, uint32_t src_x, uint32_t src_y, uint32_t pitch)
{
	...
	const uint32_t dst_x = display_x/*src_x * 4/3*/;
...
void gba_upscale_480x272(uint32_t* restrict dst, uint32_t* restrict src)
{
	uint32_t midh = 272 >> 1;
	uint32_t Eh = 0;
...
void update_display(void)
{
	...
	if (!screen_scale)
	{
		dstrect.x = (display_x - 800) / 2;
		dstrect.y = (display_y - 480) / 2;
		SDL_SoftStretch(screen, NULL, display, &dstrect);
	}
	else
	{
		...
		bitmap_scale(0, 0, 240, 160, 800, 480, screen->w, 0, (uint16_t* restrict) src, (uint16_t* restrict) display->pixels);
    ...
    for(y = 159, y2 = (160 * video_scale) - 1; y >= 0; y--)
    {
      for(i = 0; i < video_scale; i++)
      {
        memcpy(screen_ptr + (y2 * pitch),
         screen_ptr + (y * pitch), 800 * video_scale);
        y2--;
      }
    }
  }
...
//	  exit(1);
  ...
  {
    if(SDL_VideoModeOK(vmodes[vmode].x, vmodes[vmode].y, 16, SDL_HWSURFACE) != 0)
    {
      ...
      display = SDL_SetVideoMode(display_x, display_y, 16, SDL_HWSURFACE);
      break;      
    }
  }
